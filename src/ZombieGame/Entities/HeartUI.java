...
public class HeartUI extends UIElement {
    ...
    public HeartUI(double posX, double posY) {
        super(posX, posY, e -> new UIComponent(e, null) {
            ...
            public void draw() {
                ...
                int baseX = (int) this.getEntity().getPosX();
                int baseY = (int) this.getEntity().getPosY();

                for (int i = 0; i < hearts; i++) {
                    int remaining = halfHearts - (i * 2);

                    BufferedImage img;
                    if (remaining >= 2) img = full;
                    else if (remaining == 1) img = half;
                    else img = empty;

                    int x = baseX + i * (HEART_SIZE + HEART_GAP);
                    int y = baseY;
